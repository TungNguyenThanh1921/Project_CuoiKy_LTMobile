...
-- T?o b?ng Cu?c trò chuy?n (Conversation)
CREATE TABLE Conversation (
    conversation_id INT PRIMARY KEY IDENTITY(1,1),
    conversation_name VARCHAR(100),
    user_id INT,
    participant_id INT, -- Thêm c?t ?? l?u tr? thông tin ng??i tham gia cu?c trò chuy?n riêng t?
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    IsPrivate BIT NOT NULL DEFAULT 0, -- Thêm tr??ng IsPrivate ?? xác ??nh cu?c trò chuy?n có riêng t? hay không
    FOREIGN KEY (user_id) REFERENCES Users(user_id),
    FOREIGN KEY (participant_id) REFERENCES Users(user_id) -- Thêm khóa ngo?i ?? liên k?t v?i b?ng Users
);
